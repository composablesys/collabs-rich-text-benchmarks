...
export function setupAutomergeQuillBinding(
    ...
    for (const patch of patches) {
      ...
      switch (patch.action) {
        case "splice":
          ...
          // TODO: for an insertion at the end of a mark/unmark with
          // expand="after", patch.marks is incorrect (does not include the mark).
          // See https://github.com/automerge/automerge/issues/646 .
          // This apparently increases the quillOps count of Automerge docs
          // relative to other libs (2-3x), which has a perf penalty.
          // We can work around by calling marksAt, but that has an even higher
          // perf penalty (noticable for 16 vs 32 users).
          const quillFormat = automergeAttrToQuill(patch.marks ?? {});
          // const quillFormat = automergeAttrToQuill(
          //   marksAt(doc, ["text"], index)
          // );
          pendingDelta = pendingDelta.compose(
        ...
        case "mark":
          ...
          break;
        case "unmark":
          // TODO: I've never seen an UnmarkPatch; instead, we get a MarkPatch
          // with value null.
          // Reported at https://github.com/automerge/automerge/issues/715
          pendingDelta = pendingDelta.compose(
            new Delta()
              .retain(patch.start)
              .retain(patch.end - patch.start, { [patch.name]: null })
          );
          break;
        default:
  ...
  function marksAt<T>(
    ...
    const ret: Record<string, any> = {};
    // Looping over all marks is current recommended way (from Alex Good).
    for (const mark of Automerge.marks(doc, path)) {
      if (mark.start <= index && index < mark.end && mark.value !== null) {
        ret[mark.name] = mark.value;
      }
    }
    return ret;
  }
